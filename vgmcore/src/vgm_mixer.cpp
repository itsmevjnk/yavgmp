...
const float vgm_parser::chip_volumes[] = {
...
pff vgm_parser::mix_outputs() {
    pff result = std::make_pair(0.0, 0.0);
    float absvol = 0; // absolute volume
    for(int i = 0; i < sizeof(chips) / sizeof(vgm_chip*); i++) {
        vgm_chip* chip = chips_array[i];
        if(chip) {
            /* chip is available */
            pff output = chip->mix_channels();
            float volume = chip_volumes[i]; absvol += volume;
